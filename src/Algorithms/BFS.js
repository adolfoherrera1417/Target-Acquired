...
const solve = (matrix, start, endNode) => {
    ...
    var visited = []
    for(let i = 0; i < 36; i++) {
        visited.push(new Array(36).fill(false))
    }

    // create prev matrix
    var prev = []
    for(let i = 0; i < 36; i++) {
        prev.push(new Array(36).fill(null))
    }
    ...
    while(!queue.isEmpty()) {
        ...
        if ((x < 0) || (y < 0) || (x >= 36) || (y >= 36) || matrix[x][y] === "W" || visited[x][y]) {
			continue;
        }
		...
		if (y+1 < 36 && !visited[x][y+1]) {prev[x][y+1] = node;}

		queue.enqueue({x: x+1,y}); //top;
		if(x + 1 < 36 && !visited[x+1][y]) {prev[x+1][y] = node;}
